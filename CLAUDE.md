...
| 自動整形              | `pnpm format`       |
| テスト(watch)         | `pnpm test`         |
| テスト(1 回・全系統)  | `pnpm test:run`     |
| テスト(unit のみ)     | `pnpm test:unit`    |
| テスト(browser のみ)  | `pnpm test:browser` |
| アイコン再生成        | `pnpm icons`        |
...
- Vitest + Testing Library。`vite.config.ts` の `test.projects` で 2 系統に分ける。
  - **unit**(happy-dom): ロジックと、matchMedia / PWA など制御が必要な DOM テスト。`src/**/*.test.ts`(`*.browser.test.ts` を除く)。
  - **browser**(Playwright / Chromium, headless): `<dialog>` の開閉など実ブラウザが忠実なテスト。`src/**/*.browser.test.ts`。実行には `pnpm exec playwright install chromium` で Chromium が必要。
- 新しいテストはまず unit(happy-dom)に置く。matchMedia / display-mode / beforeinstallprompt に依存するものは実ブラウザだと制御しにくいので unit に残す。実ブラウザでしか忠実に検証できないもの(`<dialog>`、実 `requestAnimationFrame` など)だけ `*.browser.test.ts` にする。
